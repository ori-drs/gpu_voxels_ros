...
int main(int argc, char* argv[])
{
  ...
  while (ros::ok())
  {
    ...
    if (new_data_received) 
    {
      ...
      // std::vector<DistanceVoxel::pba_dist_t> host_dist(pbaDistanceVoxmap->getVoxelMapSize());
      // pbaDistanceVoxmap->getAllDistancesToHost(host_dist);

      // DistanceVoxel::pba_dist_t max = *std::max_element(host_dist.begin(), host_dist.end());
      // DistanceVoxel::pba_dist_t min = *std::min_element(host_dist.begin(), host_dist.end());
      ...
      LOGGING_INFO(Gpu_voxels, "calculate inverse distance map for " << countingVoxelList->getDimensions().x << " occupied voxels" << endl);
      auto start = std::chrono::high_resolution_clock::now(); 
      pbaInverseDistanceVoxmap->parallelBanding3D();
      auto finish = std::chrono::high_resolution_clock::now(); 
      auto duration = std::chrono::duration_cast<std::chrono::microseconds>(finish - start); 
      double dur = duration.count();
      LOGGING_INFO(Gpu_voxels, "Inverse calculation time: " <<  dur << " microseconds." << endl);
      // Calculate the signed distance

      std::vector<float> sdf_map(pbaDistanceVoxmap->getVoxelMapSize());
      LOGGING_INFO(Gpu_voxels, "Calculating the sdf" << endl);
      ...
      // diff_result = pbaDistanceVoxmap->differences3D(pbaInverseDistanceVoxmap, false);
      ...
      LOGGING_INFO(Gpu_voxels, "start cloning pbaInverseDistanceVoxmap" << endl);
      pbaDistanceVoxmapVisual->clone(*(pbaInverseDistanceVoxmap.get()));
      LOGGING_INFO(Gpu_voxels, "done cloning pbaInverseDistanceVoxmap" << endl);
