...
Vector3ui map_dimensions(128, 128, 64);
size_t num_voxels = 128*128*64; 

float voxel_side_length = 0.04f; // 1 cm voxel size
...
int main(int argc, char* argv[])
{
  ...
  {
    measurement_points.push_back(Vector3i(20+i, 20+i, 20+i));

  }
  
  // measurement_points.push_back(Vector3i(80, 140, 100));
  // measurement_points.push_back(Vector3i(100, 180, 126));
  ...
  while (ros::ok())
  {
    ...
    if (new_data_received) 
    {
      ...
      for(size_t i = 0; i < measurement_points.size(); i++)
      {
        ...
        auto copy_time = high_resolution_clock::now(); 

        float metric_free_space = sqrtf(dv.squaredObstacleDistance(measurement_points[i])) * voxel_side_length;
        auto stop = high_resolution_clock::now(); 

        auto copy_duration = duration_cast<microseconds>(copy_time - start); 
        auto dist_duration = duration_cast<microseconds>(stop - copy_time); 

        std::cout << "Time to copy: " << copy_duration.count() << " microseconds       " << "Time to get dist: " << dist_duration.count() << " microseconds" << std::endl; 

        // LOGGING_INFO(Gpu_voxels, "Obstacle @ " << dv.getObstacle() << " Voxel @ " << measurement_points[i] << " has a clearance of " << metric_free_space << "m." << endl);
      }

      // Copy entire voxelmap
      
      // gpu_voxels::voxelmap::DistanceVoxelMap host_voxmap[num_voxels];
      DistanceVoxelMap *map = gvl->getMap("pbaDistanceVoxmap")->as<DistanceVoxelMap>();
      DistanceVoxel* buffer = new DistanceVoxel[num_voxels];
      // DistanceVoxel host_voxmap[num_voxels];
      cudaMemcpy((void*) buffer, map->getDeviceDataPtr(), sizeof(DistanceVoxel) * num_voxels, cudaMemcpyDeviceToHost);

      auto stop = high_resolution_clock::now(); 
      auto batch_duration = duration_cast<microseconds>(stop - start); 
      std::cout << "Batch time : " << batch_duration.count() << " microseconds. " << std::endl; 

      for(size_t i = 0; i < 1000; i++)
          {

            int id = voxelmap::getVoxelIndexSigned(map_dimensions, measurement_points[1]);
            float metric_free_space = sqrtf(buffer[i].squaredObstacleDistance(measurement_points[1])) * voxel_side_length;
            // LOGGING_INFO(Gpu_voxels, "Obstacle @ " << buffer[i].getObstacle() << endl);
            // LOGGING_INFO(Gpu_voxels, "Obstacle @ " << buffer[id].getObstacle() << " Voxel @ " << measurement_points[i] << " has a clearance of " << metric_free_space << "m." << endl);
          }
      auto final_stop = high_resolution_clock::now(); 

      auto batch_query_duration = duration_cast<microseconds>(final_stop - stop); 
      std::cout << "Batch query time : " << batch_query_duration.count() << " microseconds. " << std::endl; 
    ...
    pbaDistanceVoxmap->getDistancesToHost(h_indices, distances);
